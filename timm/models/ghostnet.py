...
class GhostBottleneck(nn.Module):
    ...
    def __init__(
        ...
        else:  # plain – zwykły conv1x1 zamiast GhostModule
            self.ghost1 = nn.Sequential(
                nn.Conv2d(in_chs, mid_chs, 1, 1, 0, bias=False),
    ...
    def __init__(
        ...
        for cfg in self.cfgs:
            ...
            for k, exp_size, c, se_ratio, s in cfg:
                ...
                # Wybór trybu działania GhostBottleneck:
                # - jeśli ablacja: plain (zwykłe 1×1 conv zamiast GhostModule)
                # - jeśli wersja v2 i nie-ablacja na warstwach >1: attn (GhostModuleV2)
                # - w pozostałych przypadkach: original (GhostModule v1)
                if not self.use_ghost:
                    mode = 'plain'
                elif version == 'v2' and layer_idx > 1:
                    mode = 'attn'
                else:
                    mode = 'original'
                layers.append(
...
def _create_ghostnet(variant, width=1.0, pretrained=False, use_ghost=True, **kwargs):
    """
