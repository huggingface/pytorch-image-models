...
class PatchMix:
    ...
    def _shuffle(self, x):
        ...
        forward_indexes = torch.as_tensor(
            indexes[0], dtype=torch.long).to(x.device)
        forward_indexes = forward_indexes.repeat(b, 1)
        backward_indexes = torch.as_tensor(
            indexes[1], dtype=torch.long).to(x.device)
    ...
    def _mix(self, x, m):
        ...
        mix_x = torch.gather(
            mix_x, 0, mix_indexes.repeat(s, c, 1).permute(-1, 0, 1))
        ...
        m2m_indexes = ((ids - m + 1) + torch.arange(m *
                       2 - 1, device=x.device) + b) % b
    ...
    def __call__(self, x, target):
        ...
        if use_mix:
            ...
            x = x.reshape(b, c, n_h, p, n_w, p).permute(
                0, 2, 4, 1, 3, 5).reshape(b, n_h*n_w, c*p*p)
            ...
            x = x.reshape(b, n_h, n_w, c, p, p).permute(
                0, 3, 1, 4, 2, 5).reshape(b, c, n_h*p, n_w*p)
        else:
            m = 1
        ...
        m2m_target = one_hot(m2m_target, self.num_classes, on_value.expand(
            [m2m_target.shape[0], -1]), off_value)
