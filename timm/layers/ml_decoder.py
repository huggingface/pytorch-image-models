...
def add_ml_decoder_head(model, head_version='new', **kwargs):
    ...
    if hasattr(model, 'global_pool') and hasattr(model, 'fc'):  # most CNN models, like Resnet50
        ...
        model.forward_head = model.head.forward #types.MethodType(model.head.forward, model, type(model))
...
class MLDecoder(nn.Module):
    def __init__(
        ...
        self.norm1 = norm_layer(dim if use_input_proj else in_features)
