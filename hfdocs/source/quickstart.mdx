...
For different input channels, set `in_chans` to the model's target channel count;
the pretrained configuration must describe the source weights. See
[Adapt and Export Models](model_adaptation) for channel conversion and saving
adapted models with their current configuration.
